...
def set_up():
    ...
    stream = os.popen(
        f"az storage account show-connection-string --name {storage_account_name} --resource-group {resource}"
    )
    ...
    template_path = (
        "serwo/python/src/utils/CollectLogDirectories/CollectLogsTemplate/func.py"
    )
    ...
    return {"queue": queue_name, "connectionString": connection_str}
...
if option == "--partition":
    ...
    subprocess.call(
        ["python3", "serwo_create_multicloud_statemachine.py", user_dir, user_json, "1"]
    )
    ...
    with open(f"{user_dir}/queue_details.json", "w+") as f:
        f.write(json.dumps(queue_details, indent=4))
    subprocess.call(
        ["python3", "serwo_function_fuse.py", user_dir, user_json, fuse_csp_call]
    )
