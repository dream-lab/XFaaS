...
class UserDag:
    ...
    __nodeIDMap = (
        {}
    )  # map: nodeName -> nodeId (used internally) [NOTE [TK] - This map is changed from nodeName -> NodeId to UserGivenNodeId -> our internal nodeID ] (in azure we were already using this)
    ...
    def __init__(self, user_config_path):
        ...
        for node in self.__dag_config_data["Nodes"]:
            ...
            self.__dag.add_node(
                ...
                ret=[
                    "yield ",
                    'context.call_activity("' + node["NodeName"] + '",$var$)',
                ],
        ...
        self.__dag.nodes[start_node]["ret"] = [
            "yield ",
            'context.call_activity("'
            + self.__dag.nodes[start_node]["NodeName"]
            + '", serwoObject)',
        ]
    ...
    def _get_orchestrator_code_linear_merge(self, dag, nodes):
        ...
        for node in nodes[:-1]:
            ...
            if previous_var is not None:
                ...
                var_substituted = dag.nodes[node]["ret"][1].replace(
                    "$var$", previous_var
                )
            else:
                pre += "\n" + dag.nodes[node]["pre"]
            ...
            pre += (
                ...
                + dag.nodes[node]["var"]
                + " = "
                + dag.nodes[node]["ret"][0]
                + " "
                + var_substituted
            )
            previous_var = dag.nodes[node]["var"]

        pre += "\n" + dag.nodes[last]["pre"].replace(
            "$var$", dag.nodes[nodes[-2]]["var"]
        )
        var = self._generate_random_variable_name()
        # $var$ will be updated in linear merge
        ret = [
            "yield ",
            dag.nodes[last]["ret"][1].replace("$var$", dag.nodes[nodes[-2]]["var"]),
        ]
        return pre, ret, var
    ...
    def _merge_linear_nodes(
        ...
        outG.add_node(
            newNodeId, pre=pre, ret=ret, var=var, machine_list=new_node_machine_list
        )  # Add the 'merged' node
    ...
    def get_orchestrator_code(self):
        ...
        if len(output_dag.nodes[end_node]["pre"]) == 0:
            ...
            post_code = (
                final_var
                + " = "
                + output_dag.nodes[end_node]["ret"][0]
                + " "
                + output_dag.nodes[end_node]["ret"][1]
            )
            ...
            insert_metadata_statement = (
                variable_for_insert_metadata
                + " = "
                + f"insert_end_stats_in_metadata({variable_for_insert_metadata})"
            )
        ...
        orchestrator_code = "\n".join(
            [pre_statements[0]] + ["\t" + statement for statement in pre_statements[1:]]
        )
