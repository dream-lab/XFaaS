...
json_templates_base = (
    f"{parent_directory}/python/src/faas-templates/azure/json-templates"
)
...
starter_path = (
    f"{parent_directory}/python/src/faas-templates/azure/predefined-functions/Starter"
)
...
## Runner template
runner_template_file = (
    f"{parent_directory}/python/src/runner-templates/azure/runner_template.py"
)
runner_template_file_secondary = (
    f"{parent_directory}/python/src/runner-templates/azure/runner_template_secondary.py"
)
...
def populate_orchestrator():
    stream = os.popen(
        f"python3 {orchestrator_generator_path} {user_json_dir} {json_file_name} {TEMP_TRIGGER}"
    )
...
def copy_meta_files(user_fns_data, ingress_queue_name, user_app_name):
    ...
    shutil.copyfile(
        resources_json, f"{az_functions_path}/QueueTrigger/azure_resources.json"
    )
    ...
    for dir in dirs:
        ...
        if ".py" not in dir:
            if not os.path.exists(fin_func_dir + "/" + dir):
                shutil.copytree(
                    fn_dir_path + "/" + dir, fin_func_dir + "/" + dir, False, None
                )
            ...
            for f in in_dir:
                if ".py" in f:
                    ...
                    fr.f_and_r(
                        path, fin_func_dir + "/" + dir + "/" + f, str_find, str_replace
                    )
                    # stream = os.popen(f"python3 find_and_replace.py {path} {path} {str_find} {str_replace}")
...
if __name__ == "__main__":
    ...
    (
        ingress_queue_name,
        app_name,
    ) = generate_app_name_and_populate_and_get_ingress_queue_name(user_app_name)
