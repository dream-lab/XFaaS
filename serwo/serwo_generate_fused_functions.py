...
class FusionCodeGenerator:
    ...
    def __init__(self, dag_to_fuse):
        ...
        for node in dag_to_fuse.nodes():
            ...
            dag_to_fuse.nodes[node]["code"] = (
                output_var
                + " = "
                + dag_to_fuse.nodes[node]["NodeName"]
                + ".function($input_var$)"
                + "\n\t"
        ...
        start_node = [node for node in dag_to_fuse if dag_to_fuse.in_degree(node) == 0][
            0
        ]

        dag_to_fuse.nodes[start_node]["code"] = (
            dag_to_fuse.nodes[start_node]["output_var"]
            + " = "
            + dag_to_fuse.nodes[start_node]["NodeName"]
            + ".function(serwoObject)"
            + "\n\t"
            + dag_to_fuse.nodes[start_node]["output_var"]
    ...
    def _get_fusion_code_linear_merge(self, dag, nodes):
        ...
        for node in nodes[:-1]:
            ...
            if previous_var is not None:
                code += "\n\t" + dag.nodes[node]["code"].replace(
                    "$input_var$", previous_var
                )
            else:
                code += "\n\t" + dag.nodes[node]["code"]
            previous_var = dag.nodes[node]["output_var"]
        code += "\n\t" + dag.nodes[last]["code"].replace(
            "$input_var$", dag.nodes[nodes[-2]]["output_var"]
        )
    ...
    def _merge_linear_nodes(
        ...
        input_var, code, output_var = self._get_fusion_code_linear_merge(
            outG, node_list
        )

        outG.add_node(
            newNodeId, input_var=input_var, code=code, output_var=output_var
        )  # Add the 'merged' node
    ...
    def _merge_parallel_nodes(
        ...
        newNodeId = "n" + str(xd)
        # add new node pre
        input_var, code, output_var = self._get_fusion_code_parallel_merge(
            outG, node_list
        )
    ...
    def get_fused_code(self):
        ...
        for node in output_dag:
            return (
                output_dag.nodes[node]["code"]
                + "\n\treturn "
                + output_dag.nodes[node]["output_var"]
            )
