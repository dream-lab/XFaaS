...
def calculate_sub_dag_latency(csp_id, nodes, sub_dag):
    for node in nodes:
        ...
        for succ in successors:
            edge_latency = sub_dag.edges[(node, succ)]["EdgeBenchmark"]["Latencies"][
                csp_id
            ][csp_id]
    ...
    for src in sources:
        ...
        for index in range(0, len(critical_path) - 1):
            path_latency += sub_dag[critical_path[index]][critical_path[index + 1]][
                "edge_latency"
            ]
...
def evaluate_node_and_edge_constraints(csp_id, sub_dag, user_pinned_nodes):
    ...
    for ed in sub_dag.edges:
        if (
            sub_dag.edges[ed]["EdgeBenchmark"]["DataTransferSize"] > 256
            and csp_id == "0"
        ):
            flag = True
            break
...
def populate_benchmarks_for_user_dag(
    user_dag,
    user_dir,
    user_pinned_nodes,
    benchmark_path,
    ...
    bm_data, edges, latency_map, user_dag_copy = init_benchmark_populator(
        benchmark_path, user_dag, cloud_ids
    )

    latency_benchmark = populate_latanecy_benchmarks(
        bm_data,
        cloud_ids,
        edges,
        latency_map,
    ...
    for nd in valid_partition_points:
        ...
        for sc in successors:
            inter_cloud_data_transfer.append(
                user_dag_copy.edges[(nd["node_id"], sc)]["EdgeBenchmark"][
                    "DataTransferSize"
                ]
            )
            break
...
def populate_latanecy_benchmarks(
    bm_data,
    cloud_ids,
    edges,
    latency_map,
