...
def partition_dag(
    user_dag, user_dir, user_dag_input, user_pinned_nodes, benchmark_path
):
    ...
    (
        ...
        is_fan_in,
    ) = xfaas_benchmark.populate_benchmarks_for_user_dag(
        user_dag,
        user_dir,
        user_pinned_nodes,
        benchmark_path,
    ...
    st = time.time()
    if opt == "DP":
        clouds, min_latency = dp_xfaas_partitioner.get_optimal_partitions(
            latencies_benchmark,
            data_transfers_benchmark,
            inter_cloud_data_tranfers,
            is_fan_in,
        ...
        if min_latency == sys.maxsize:
            print(
                "DAG CANNOT BE PARTITIONED FOR GIVEN INPUT VALUES AND USER CONSTRAINTS"
            )
        print(
            "Result from DP partitioner: \n clouds -> ",
            clouds,
            "\n Latency -> ",
            min_latency,
        )
        ...
        clouds, min_latency = ilp_xfaas_partitioner.get_optimal_partitions(
            latencies_benchmark,
            data_transfers_benchmark,
            inter_cloud_data_tranfers,
            is_fan_in,
...
def fuse(user_dag, user_dag_input, user_dir):
    csp = "AWS"
    wf_name, wf_id, refactored_wf_id, wf_deployment_id, dag_path = fusion_wrapper.run(
        user_dag, user_dir, user_dag_input, csp
    )
