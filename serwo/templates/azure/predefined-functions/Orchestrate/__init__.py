...
def orchestrator_function(context: df.DurableOrchestrationContext):
    ...
    # user dag execution
    oyyh = yield context.call_activity("Trigger", serwoObject)
    twql = yield context.call_activity("TextFileData2", oyyh)
    qykz = yield context.call_activity("TextFileData9", oyyh)
    apgo = yield context.call_activity("TextFileData5", oyyh)
    rfms = yield context.call_activity("TextFileData8", oyyh)
    dbah = yield context.call_activity("TextFileData6", oyyh)
    swwh = yield context.call_activity("TextFileData3", oyyh)
    yqei = yield context.call_activity("TextFileData10", oyyh)
    ctru = yield context.call_activity("TextFileData1", oyyh)
    gzrh = yield context.call_activity("TextFileData7", oyyh)
    gewy = yield context.call_activity("TextFileData4", oyyh)
    vlsq = []
    dfsx = context.call_activity("TextSort2", twql)
    hbtf = context.call_activity("TextSort9", qykz)
    hrvw = context.call_activity("TextSort5", apgo)
    osrh = context.call_activity("TextSort8", rfms)
    uaer = context.call_activity("TextSort6", dbah)
    mzra = context.call_activity("TextSort3", swwh)
    jioh = context.call_activity("TextSort10", yqei)
    xgco = context.call_activity("TextSort", ctru)
    cbjx = context.call_activity("TextSort7", gzrh)
    qlok = context.call_activity("TextSort4", gewy)
    vlsq.append(dfsx)
    vlsq.append(hbtf)
    vlsq.append(hrvw)
    vlsq.append(osrh)
    ...
    vlsq.append(cbjx)
    vlsq.append(qlok)
    vjgb = yield context.task_all(vlsq)
    gfgj = yield context.call_activity("AggregateLines", vjgb)
    tbtp = yield context.call_activity("MergeAggregate", gfgj)
    emay = yield context.call_activity("SingleString", tbtp)
    qizm = yield context.call_activity("Encrypt", emay)
    qizm = insert_end_stats_in_metadata(qizm)
    mzgq = yield context.call_activity("CollectLogs", qizm)
    return mzgq
