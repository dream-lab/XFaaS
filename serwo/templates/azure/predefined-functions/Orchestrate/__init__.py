...
def orchestrator_function(context: df.DurableOrchestrationContext):
    ...
    # user dag execution
    nunv = yield context.call_activity("Trigger", serwoObject)
    mlhi = yield context.call_activity("TextFileData1", nunv)
    jwdh = yield context.call_activity("TextFileData9", nunv)
    dgca = yield context.call_activity("TextFileData6", nunv)
    ugpm = yield context.call_activity("TextFileData7", nunv)
    fjop = yield context.call_activity("TextFileData3", nunv)
    ncxg = yield context.call_activity("TextFileData10", nunv)
    lhof = yield context.call_activity("TextFileData5", nunv)
    tkje = yield context.call_activity("TextFileData4", nunv)
    zpts = yield context.call_activity("TextFileData2", nunv)
    lbcm = yield context.call_activity("TextFileData8", nunv)
    lagv = []
    ixeq = context.call_activity("TextSort", mlhi)
    icbf = context.call_activity("TextSort9", jwdh)
    nhkb = context.call_activity("TextSort6", dgca)
    lwoi = context.call_activity("TextSort7", ugpm)
    plqy = context.call_activity("TextSort3", fjop)
    cubn = context.call_activity("TextSort10", ncxg)
    rjnl = context.call_activity("TextSort5", lhof)
    aezy = context.call_activity("TextSort4", tkje)
    zoip = context.call_activity("TextSort2", zpts)
    kkqa = context.call_activity("TextSort8", lbcm)
    lagv.append(ixeq)
    lagv.append(icbf)
    lagv.append(nhkb)
    lagv.append(lwoi)
    ...
    lagv.append(zoip)
    lagv.append(kkqa)
    pdpz = yield context.task_all(lagv)
    soby = yield context.call_activity("AggregateLines", pdpz)
    yvex = yield context.call_activity("MergeAggregate", soby)
    togr = yield context.call_activity("SingleString", yvex)
    cocx = yield context.call_activity("Encrypt", togr)
    cocx = insert_end_stats_in_metadata(cocx)
    ulyx = yield context.call_activity("CollectLogs", cocx)
    return ulyx
