...
def orchestrator_function(context: df.DurableOrchestrationContext):
    ...
    # user dag execution
    qgya = yield context.call_activity("fetchData", serwoObject)
    uaiy = yield context.call_activity("grayscale", qgya)
    pcvi = yield context.call_activity("flip", uaiy)
    krzt = yield context.call_activity("rotate", pcvi)
    geno = yield context.call_activity("resize", krzt)
    qhus = []
    qwmx = context.call_activity("resnet", geno)
    rfno = context.call_activity("alexnet", geno)
    jwtx = context.call_activity("mobilenet", geno)
    qhus.append(qwmx)
    qhus.append(rfno)
    qhus.append(jwtx)
    shue = yield context.task_all(qhus)
    stuq = yield context.call_activity("aggregator", shue)
    stuq = insert_end_stats_in_metadata(stuq)
    sbsj = yield context.call_activity("CollectLogs", stuq)
    return sbsj
