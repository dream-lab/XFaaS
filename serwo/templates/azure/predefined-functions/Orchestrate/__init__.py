...
def orchestrator_function(context: df.DurableOrchestrationContext):
    ...
    # user dag execution
    jnqz = yield context.call_activity("Trigger", serwoObject)
    wina = yield context.call_activity("TextFileData2", jnqz)
    hjth = yield context.call_activity("TextFileData10", jnqz)
    tsol = yield context.call_activity("TextFileData7", jnqz)
    oefh = yield context.call_activity("TextFileData9", jnqz)
    kevx = yield context.call_activity("TextFileData6", jnqz)
    zyrj = yield context.call_activity("TextFileData3", jnqz)
    ipgh = yield context.call_activity("TextFileData8", jnqz)
    hmmq = yield context.call_activity("TextFileData1", jnqz)
    xnlg = yield context.call_activity("TextFileData4", jnqz)
    luwt = yield context.call_activity("TextFileData5", jnqz)
    qthi = []
    gdyi = context.call_activity("TextSort2", wina)
    fhba = context.call_activity("TextSort10", hjth)
    afrh = context.call_activity("TextSort7", tsol)
    rjgy = context.call_activity("TextSort9", oefh)
    smis = context.call_activity("TextSort6", kevx)
    kspw = context.call_activity("TextSort3", zyrj)
    oqdj = context.call_activity("TextSort8", ipgh)
    yisa = context.call_activity("TextSort", hmmq)
    omcn = context.call_activity("TextSort4", xnlg)
    gjpe = context.call_activity("TextSort5", luwt)
    qthi.append(gdyi)
    qthi.append(fhba)
    qthi.append(afrh)
    qthi.append(rjgy)
    ...
    qthi.append(omcn)
    qthi.append(gjpe)
    mlfa = yield context.task_all(qthi)
    sxlp = yield context.call_activity("AggregateLines", mlfa)
    sfbb = yield context.call_activity("MergeAggregate", sxlp)
    hxtm = yield context.call_activity("SingleString", sfbb)
    pkyo = yield context.call_activity("Encrypt", hxtm)
    pkyo = insert_end_stats_in_metadata(pkyo)
    keui = yield context.call_activity("CollectLogs", pkyo)
    return keui
