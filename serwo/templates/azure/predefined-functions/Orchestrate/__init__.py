...
def orchestrator_function(context: df.DurableOrchestrationContext):
    ...
    # user dag execution
    dxob = yield context.call_activity("Trigger", serwoObject)
    drle = yield context.call_activity("TextFileData5", dxob)
    bwgz = yield context.call_activity("TextFileData10", dxob)
    xxtf = yield context.call_activity("TextFileData2", dxob)
    mogv = yield context.call_activity("TextFileData8", dxob)
    ldxa = yield context.call_activity("TextFileData6", dxob)
    inun = yield context.call_activity("TextFileData7", dxob)
    dpqg = yield context.call_activity("TextFileData4", dxob)
    mfqw = yield context.call_activity("TextFileData3", dxob)
    wmbi = yield context.call_activity("TextFileData1", dxob)
    rbat = yield context.call_activity("TextFileData9", dxob)
    kdkp = []
    wckd = context.call_activity("TextSort5", drle)
    xdpj = context.call_activity("TextSort10", bwgz)
    gssq = context.call_activity("TextSort2", xxtf)
    madw = context.call_activity("TextSort8", mogv)
    hvuk = context.call_activity("TextSort6", ldxa)
    bxxf = context.call_activity("TextSort7", inun)
    pnny = context.call_activity("TextSort4", dpqg)
    ffei = context.call_activity("TextSort3", mfqw)
    ngpw = context.call_activity("TextSort", wmbi)
    ildm = context.call_activity("TextSort9", rbat)
    kdkp.append(wckd)
    kdkp.append(xdpj)
    kdkp.append(gssq)
    kdkp.append(madw)
    ...
    kdkp.append(ngpw)
    kdkp.append(ildm)
    gtdd = yield context.task_all(kdkp)
    shax = yield context.call_activity("AggregateLines", gtdd)
    bykq = yield context.call_activity("MergeAggregate", shax)
    bqsi = yield context.call_activity("SingleString", bykq)
    gqqt = yield context.call_activity("Encrypt", bqsi)
    gqqt = insert_end_stats_in_metadata(gqqt)
    elke = yield context.call_activity("CollectLogs", gqqt)
    return elke
