...
def orchestrator_function(context: df.DurableOrchestrationContext):
    ...
    # user dag execution
    xelm = yield context.call_activity("fetch_data", serwoObject)
    bioq = yield context.call_activity("grayscale", xelm)
    lewl = yield context.call_activity("flip", bioq)
    fbta = yield context.call_activity("rotate", lewl)
    qshf = yield context.call_activity("resize", fbta)
    ixsz = []
    qraq = context.call_activity("resnet", qshf)
    xjxg = context.call_activity("alexnet", qshf)
    uuwp = context.call_activity("mobilenet", qshf)
    ixsz.append(qraq)
    ixsz.append(xjxg)
    ixsz.append(uuwp)
    occi = yield context.task_all(ixsz)
    ided = yield context.call_activity("aggregator", occi)
    ided = insert_end_stats_in_metadata(ided)
    qpgl = yield context.call_activity("CollectLogs", ided)
    return qpgl
