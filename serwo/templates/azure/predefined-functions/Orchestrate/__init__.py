...
<<<<<<< HEAD
    ...
    gwbt = yield context.call_activity("TextFileData3", etcn)
    suat = yield context.call_activity("TextFileData10", etcn)
    tesp = yield context.call_activity("TextFileData6", etcn)
    uxrc = yield context.call_activity("TextFileData7", etcn)
    mfzy = yield context.call_activity("TextFileData9", etcn)
    okfs = yield context.call_activity("TextFileData5", etcn)
    hhnc = yield context.call_activity("TextFileData1", etcn)
    aykc = yield context.call_activity("TextFileData8", etcn)
    thng = []
    yfog = context.call_activity("TextSort4", nlna)
    kbor = context.call_activity("TextSort2", dgze)
    vgtl = context.call_activity("TextSort3", gwbt)
    whkx = context.call_activity("TextSort10", suat)
    miem = context.call_activity("TextSort6", tesp)
    woqq = context.call_activity("TextSort7", uxrc)
    xttg = context.call_activity("TextSort9", mfzy)
    xttk = context.call_activity("TextSort5", okfs)
    oezw = context.call_activity("TextSort", hhnc)
    gqri = context.call_activity("TextSort8", aykc)
    thng.append(yfog)
    thng.append(kbor)
    thng.append(vgtl)
    thng.append(whkx)
    ...
    thng.append(oezw)
    thng.append(gqri)
    nqhw = yield context.task_all(thng)
    tlxy = yield context.call_activity("AggregateLines", nqhw)
    wusn = yield context.call_activity("MergeAggregate", tlxy)
    moog = yield context.call_activity("SingleString", wusn)
    waaa = yield context.call_activity("Encrypt", moog)
