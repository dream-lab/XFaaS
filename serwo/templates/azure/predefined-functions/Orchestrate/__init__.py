...
def orchestrator_function(context: df.DurableOrchestrationContext):
    ...
    # user dag execution
    wysc = yield context.call_activity("ForwardFunction", serwoObject)
    gfux = yield context.call_activity("TextFileData8", wysc)
    gbnx = yield context.call_activity("TextFileData2", wysc)
    swmh = yield context.call_activity("TextFileData9", wysc)
    jgkn = yield context.call_activity("TextFileData4", wysc)
    dfdd = yield context.call_activity("TextFileData1", wysc)
    yqgh = yield context.call_activity("TextFileData3", wysc)
    nvqq = yield context.call_activity("TextFileData6", wysc)
    isoe = yield context.call_activity("TextFileData5", wysc)
    hcbr = yield context.call_activity("TextFileData7", wysc)
    fnjz = yield context.call_activity("TextFileData10", wysc)
    mgnh = []
    qopj = context.call_activity("TextSort8", gfux)
    lkke = context.call_activity("TextSort2", gbnx)
    ugki = context.call_activity("TextSort9", swmh)
    gsfe = context.call_activity("TextSort4", jgkn)
    wuib = context.call_activity("TextSort", dfdd)
    bfzd = context.call_activity("TextSort3", yqgh)
    nvnx = context.call_activity("TextSort6", nvqq)
    qkff = context.call_activity("TextSort5", isoe)
    iddl = context.call_activity("TextSort7", hcbr)
    osjn = context.call_activity("TextSort10", fnjz)
    mgnh.append(qopj)
    mgnh.append(lkke)
    mgnh.append(ugki)
    mgnh.append(gsfe)
    ...
    mgnh.append(iddl)
    mgnh.append(osjn)
    jomk = yield context.task_all(mgnh)
    kvxh = yield context.call_activity("AggregateLines", jomk)
    kvxh = insert_end_stats_in_metadata(kvxh)
    pamh = yield context.call_activity("PushToSQS", kvxh)
    return pamh
