...
def orchestrator_function(context: df.DurableOrchestrationContext):
    ...
    # user dag execution
    ctpv = yield context.call_activity("Trigger", serwoObject)
    nnmx = yield context.call_activity("TextFileData3", ctpv)
    llts = yield context.call_activity("TextFileData5", ctpv)
    fyum = yield context.call_activity("TextFileData1", ctpv)
    qzpw = yield context.call_activity("TextFileData4", ctpv)
    kuhc = yield context.call_activity("TextFileData7", ctpv)
    czbn = yield context.call_activity("TextFileData2", ctpv)
    ajpm = yield context.call_activity("TextFileData6", ctpv)
    mnbd = yield context.call_activity("TextFileData9", ctpv)
    ftlx = yield context.call_activity("TextFileData8", ctpv)
    gsgh = yield context.call_activity("TextFileData10", ctpv)
    vwca = []
    kaer = context.call_activity("TextSort3", nnmx)
    swln = context.call_activity("TextSort5", llts)
    woil = context.call_activity("TextSort", fyum)
    xtat = context.call_activity("TextSort4", qzpw)
    wpft = context.call_activity("TextSort7", kuhc)
    rwrs = context.call_activity("TextSort2", czbn)
    xbwx = context.call_activity("TextSort6", ajpm)
    lqef = context.call_activity("TextSort9", mnbd)
    fcmx = context.call_activity("TextSort8", ftlx)
    zywy = context.call_activity("TextSort10", gsgh)
    vwca.append(kaer)
    vwca.append(swln)
    vwca.append(woil)
    vwca.append(xtat)
    ...
    vwca.append(fcmx)
    vwca.append(zywy)
    tvwc = yield context.task_all(vwca)
    ymmp = yield context.call_activity("AggregateLines", tvwc)
    pmer = yield context.call_activity("MergeAggregate", ymmp)
    ycyj = yield context.call_activity("SingleString", pmer)
    edmh = yield context.call_activity("Encrypt", ycyj)
    edmh = insert_end_stats_in_metadata(edmh)
    japl = yield context.call_activity("CollectLogs", edmh)
    return japl
