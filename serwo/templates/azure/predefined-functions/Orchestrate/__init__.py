...
def orchestrator_function(context: df.DurableOrchestrationContext):
    ...
    # user dag execution
    aisz = yield context.call_activity("Trigger", serwoObject)
    hooy = yield context.call_activity("TextFileData5", aisz)
    eclm = yield context.call_activity("TextFileData8", aisz)
    hzxm = yield context.call_activity("TextFileData10", aisz)
    dfwc = yield context.call_activity("TextFileData3", aisz)
    prpa = yield context.call_activity("TextFileData6", aisz)
    drma = yield context.call_activity("TextFileData1", aisz)
    mpqf = yield context.call_activity("TextFileData4", aisz)
    bnwt = yield context.call_activity("TextFileData2", aisz)
    eeiy = yield context.call_activity("TextFileData9", aisz)
    eeut = yield context.call_activity("TextFileData7", aisz)
    fvla = []
    warr = context.call_activity("TextSort5", hooy)
    oukr = context.call_activity("TextSort8", eclm)
    sxjx = context.call_activity("TextSort10", hzxm)
    rgpn = context.call_activity("TextSort3", dfwc)
    hoeh = context.call_activity("TextSort6", prpa)
    mgkj = context.call_activity("TextSort", drma)
    gezb = context.call_activity("TextSort4", mpqf)
    sjnq = context.call_activity("TextSort2", bnwt)
    wfbx = context.call_activity("TextSort9", eeiy)
    yixt = context.call_activity("TextSort7", eeut)
    fvla.append(warr)
    fvla.append(oukr)
    fvla.append(sxjx)
    fvla.append(rgpn)
    ...
    fvla.append(wfbx)
    fvla.append(yixt)
    odup = yield context.task_all(fvla)
    mlnh = yield context.call_activity("AggregateLines", odup)
    zcld = yield context.call_activity("MergeAggregate", mlnh)
    mqik = yield context.call_activity("SingleString", zcld)
    qjmw = yield context.call_activity("Encrypt", mqik)
    qjmw = insert_end_stats_in_metadata(qjmw)
    bpxq = yield context.call_activity("CollectLogsFileAz", qjmw)
    return bpxq
