...
def get_optimal_partitions(latencies_benchmark, data_transfers_benchmark,data_transfers,is_fan_in):
    ...
    for i in range(1,n):
        for j in range(0,k):
            min_col = sys.maxsize
            for x in range(0,k):
                if(dp[i-1][x][j] != sys.maxsize):
                    min_col = min(min_col,dp[i-1][x][j])
            for v in range(0,k):
                data_tranfer_latency = get_data_transfer_value(data_transfers_benchmark, i, j, v, data_transfers,is_fan_in)
...
get_optimal_partitions([[5,8,14,2],[3,11,10,7]],[[0,12],[1,0]],[64,64,64],[False,False,False,False])
