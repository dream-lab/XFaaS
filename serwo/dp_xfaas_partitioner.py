...
def get_optimal_partitions(
    latencies_benchmark, data_transfers_benchmark, data_transfers, is_fan_in
):
    ...
    for i in range(1, n):
        for j in range(0, k):
            ...
            for v in range(0, k):
                data_tranfer_latency = float(
                    get_data_transfer_value(
                        data_transfers_benchmark, i, j, v, data_transfers, is_fan_in
                    )
                )
                if (
                    data_tranfer_latency != sys.maxsize
                    and latencies_benchmark[v][i] != sys.maxsize
                    and min_col != sys.maxsize
                ):
                    dp[i][j][v] = (
                        min_col + data_tranfer_latency + latencies_benchmark[v][i]
                    )
                else:
...
def get_data_transfer_value(
    data_transfers_benchmark, i, j, v, data_tranfers, is_fan_in
):
    constraints_violated = evaluate_inter_cloud_data_transfer_constraints(
        data_tranfers, i, j, v
    )
