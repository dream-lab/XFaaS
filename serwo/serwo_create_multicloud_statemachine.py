...
def template_push_to_queue(user_source_dir: str, egress_fn_name: str, egress_fn_entrypoint: str, resource_dict: dict, aws_credentials=None):
    ...
    if resource_dict["filename"] == "azure_resources.json":
        with open(resource_dict["filepath"], "r") as fin:
            ...
            try:
                output = template.render(queue_name=queue_name,
...
def deploy_subdag(csp: CSP, user_dir: str, dag_definition_file: str,  serwo_root_dir: str, resource_dict: dict, egress_fn_details: dict):
    ...
    if csp == CSP.AWS:
        if egress_fn_details is None:
            ...
            try:
                ...
                os.system(f"python3 {serwo_root_dir}/aws_create_statemachine.py {user_dir} {dag_definition_file} REST")
    ...
    if csp == CSP.AZURE:
        ...
        os.system(f"python3 {serwo_root_dir}/scripts/azure/azure_resource_creation.py {user_dir} {dag_definition_file}")
        os.system(f"python3 {serwo_root_dir}/scripts/azure/azure_builder.py {user_dir} {dag_definition_file}")
        os.system(f"python3 {serwo_root_dir}/scripts/azure/azure_deploy.py {user_dir} {dag_definition_file}")


def add_forward_node_in_userdir(partition_point: PartitionPoint, user_dir, serwo_root_dir):
    forward_fn_template_path = f"{serwo_root_dir}/python/src/faas-templates/commons/ForwardFunction"
    user_source_dir = f"{user_dir}/src"
    os.system(f"cp -r {forward_fn_template_path} {user_source_dir}")
    return dict(NodeId="250",
...
'''
def add_egress_node_in_userdir(partition_point: PartitionPoint, user_dir, serwo_root_dir):
    left_csp = partition_point.get_left_csp()
    right_csp = partition_point.get_right_csp()
    egress_fn_id, egress_fn_name, egress_fn_entrypoint, egress_fn_template_path = get_egress_function_details(
                                                                                    left_csp=left_csp,
                                                                                    right_csp=right_csp,
    ...
    return dict(NodeId=egress_fn_id,
...
def create_dag_description(workflow_owner: str, workflow_id: str, workflow_version: str, package_url: str,  workflow_name: str, graph: nx.DiGraph, csp: CSP, output_dir: str,wf_description:str):
...
def generate_refactored_workflow(left,right,user_dir,refactored_wf_id,wf_id):
    ...
    fin_parts = [{'partition_label':left_dc,'function_ids':lp},{'partition_label':right_dc,'function_ids':rp}]
...
def evaluate_generic_partitioner():
    ...
    partition_points_ = serwo_user_dag.get_best_partition(partition_points=all_valid_partition_points,
                                                          num_parts=NUM_PARTS,
...
if __name__ == "__main__":
    ...
    if NUM_PARTS < 3:
        valid_partition_points_after_user_pinning = \
            serwo_user_dag.get_partition_points_after_user_pinning(partition_points=all_valid_partition_points,
                                                                   user_pinned_nodes=user_pinned_nodes,
                                                                   num_parts = NUM_PARTS,
                                                                   user_pinned_csp=user_pinned_csp)


        partition_point = serwo_user_dag.get_best_partition(partition_points=valid_partition_points_after_user_pinning,
                                                            num_parts=NUM_PARTS,
                                                            dag_path=DAG_BENCHMARK_PATH,
                                                            user_pinned_csp=user_pinned_csp,
                                                            user_pinned_nodes=user_pinned_nodes)
