    ...
    for m in range(0, num_parts - 2):
        for i in range(0, num_csps):
            for j in range(0, num_csps):
                mdl.add_if_then(
                    P[m, m + 1, i, j] == 1,
                    mdl.sum(P[m + 1, m + 2, j, k] for k in range(0, num_csps)) == 1,
                )

    mdl.minimize(
        mdl.sum(
            P[m, m + 1, i, j] * data_transfers_benchmark[i][j]
            for m in range(0, num_parts - 1)
            for i in range(0, num_csps)
            for j in range(0, num_csps)
        )
        + mdl.sum(
            P[m, m + 1, i, j] * latencies_benchmark[i][m]
            for m in range(0, num_parts - 1)
            for i in range(0, num_csps)
            for j in range(0, num_csps)
        )
        + mdl.sum(
            P[num_parts - 2, num_parts - 1, i, j]
            * latencies_benchmark[j][num_parts - 1]
            for i in range(0, num_csps)
            for j in range(0, num_csps)
        )
    )
...
latencies_benchmark = [
    [33, 4139, 3095, 14963.68, 3095, 33],
    [10, 514, 1473, 13816, 1473, 10],
]
...
print(
    get_optimal_partitions(
        latencies_benchmark, data_transfer_benchmark, data_transfers, is_fan_in
    )
)
