...
if __name__ == "__main__":
    ...
    if 'rps' in wf:
        ...
        labels = ['AWS SM','AzS SM','AzN SM','AWS ME','AzS ME','AzN ME','AWS LA','AzS LA','AzN LA']
...
if 'rps' in wf:
    ...
    fig.savefig(f'war_room/data_transfer_{wf}_rps1.{format}',bbox_inches='tight')
else:
    fig, ax = plt.subplots()
    fig.set_dpi(400)
    fig.set_figwidth(5)
    ...
    bplot = ax.bar([x+1 for x in range(0, len(labels))],med_l,width=0.2)

    ax.text(6.75,0.3,'NA', rotation=90, fontdict=fontdict,color='r')

    if 'image' in wf:
        ax.text(7.75,0.3,'DNF', rotation=90, fontdict=fontdict,color='r')
    ...
    fig.savefig(f'war_room/data_transfer_{wf}_payload1.{format}',bbox_inches='tight')
