...
def plot_step_graph_all_csps(wf, dynamism, colors, small_x, small_y, small_y2, aws_legend, az_legend, azv2_legend):
    fig, ax = plt.subplots()
    ...
    ax.set_ylim(ymin=0,ymax=125)
    yticks = [0,25,50,75,100,125]
    yticks2 = [int(y//1.25) for y in yticks]
    print(yticks)
    if not yticks == []:
    ...
    for i in range(0,len(small_x)):
        print(colors[i])
        l1 = ax.plot(small_x[i],small_y[i],color =colors[i],label=label1, zorder=zorders[i])
    ...
    for i in range(0,len(small_y2)):
        print(colors[i])
        l2 = ax2.plot(small_x[i],small_y2[i],color=colors_global[i], linestyle='--',label=label2, zorder=zs[i])
    ...
    plt.savefig(f"other_wfs_scaling_plot/{wf}_step.pdf", bbox_inches='tight')

def plot_sawtooth_graph_all_csps(OVERLAYS, split, wf, dynamism, colors, medium_x, medium_y, medium_y2):
    fig, ax = plt.subplots()
    ax.set_ylabel("Sliding Window Avg. E2E Time (secs)",fontsize=16)
    ax.yaxis.set_minor_locator(tck.AutoMinorLocator())
    ax.xaxis.set_minor_locator(tck.AutoMinorLocator())
    ...
    for i in range(0,len(medium_x)):
        ...
        split1,split2,split3 = split(medium_x[i],medium_y[i],OVERLAYS[(i)*3+1])
        tchs= []
        yy = []
        for j in range(0,len(split1[1])):
            if split1[1][j]>=80:
                tchs.append(split1[0][j])
                yy.append(split1[1][j])
        for t in range(len(tchs)):
            ax.text(tchs[t]-10,80,int(yy[t]), c='blue', fontsize=14)
            break
        ax.plot(split1[0],split1[1],color =colors[i],zorder=zorders[i])

        tchs= []
        yy = []
        for j in range(0,len(split2[1])):
            if split2[1][j]>=80:
                tchs.append(split2[0][j])
                yy.append(split2[1][j])
        # for t in range(len(tchs)):
        #     ax.text(tchs[t]-10,80,int(yy[t]),c='blue', fontsize=14)
        ax.plot(split2[0],split2[1],color =colors[i],zorder=zorders[i])
        ax.plot(split3[0],split3[1],color =colors[i],zorder=zorders[i])
    ...
    for i in range(0,len(medium_y2)):
        split1,split2,split3 = split(medium_x[i],medium_y2[i],OVERLAYS[3*(i)+1])
        ax2.plot(split1[0],split1[1],color =colors_global[i],linestyle='--',zorder=zs[i])
        ax2.plot(split2[0],split2[1],color =colors_global[i],linestyle='--',zorder=zs[i])
        ax2.plot(split3[0],split3[1],color =colors_global[i],linestyle='--',zorder=zs[i])
        ax2.vlines(x=split1[0][-1],ymin=0,ymax=split1[1][-1], color=colors_global[i], linestyle='--',zorder=zs[i])
        ax2.vlines(x=split2[0][-1],ymin=0,ymax=split2[1][-1], color=colors_global[i], linestyle='--',zorder=zs[i])
        ax2.hlines(xmin=split1[0][-1],xmax=split2[0][0],y=0, color=colors_global[i], linestyle='--',zorder=zs[i])
        ax2.hlines(xmin=split2[0][-1],xmax=split3[0][0],y=0, color=colors_global[i], linestyle='--',zorder=zs[i])
        ax2.vlines(x=split2[0][0],ymin=0,ymax=split2[1][0], color=colors_global[i], linestyle='--',zorder=zs[i])
        ax2.vlines(x=split3[0][0],ymin=0,ymax=split3[1][0], color=colors_global[i], linestyle='--',zorder=zs[i])
    ...
    split1,split2,split3 = split(medium_x[0],OVERLAYS[1],OVERLAYS[1])
    ax3.plot(split1[0],split1[1],color='red',zorder=6)
    ax3.plot(split2[0],split2[1],color='red',zorder=6)
    ax3.plot(split3[0],split3[1],color='red',zorder=6)

    ax3.hlines(xmin=split1[0][-1],xmax=split2[0][0],y=0, color='red', linestyle='-',linewidth=2,zorder=6)
    ax3.hlines(xmin=split2[0][-1],xmax=split3[0][0],y=0, color='red', linestyle='-',linewidth=2,zorder=6)

    ax3.vlines(x=split1[0][-1],ymin=0,ymax=split1[1][-1], color='red', linestyle='-',linewidth=2,zorder=6)
    ax3.vlines(x=split2[0][-1],ymin=0,ymax=split2[1][-1], color='red', linestyle='-',linewidth=2,zorder=6)
    
    ax3.vlines(x=split2[0][0],ymin=0,ymax=1, color='red', linestyle='-',linewidth=2,zorder=6)
    ax3.vlines(x=split3[0][0],ymin=0,ymax=1, color='red', linestyle='-',linewidth=2,zorder=6)

    
    plt.savefig(f"other_wfs_scaling_plot/{wf}_sawtooth.pdf", bbox_inches='tight')

def plot_alibaba_graph_all_csps(wf, dynamism, colors, large_x, large_y, large_y2):
    fig, ax = plt.subplots()
    ax.set_ylabel("Sliding Window Avg. E2E Time (secs)",fontsize=16)
    ax.yaxis.set_minor_locator(tck.AutoMinorLocator())
    ax.xaxis.set_minor_locator(tck.AutoMinorLocator())
    ...
    yticks = [0,20,40,60,80]
    yticks2 = [int(y*2.5) for y in yticks]
    if not yticks == []:
        ax.set_yticks(yticks)
        ax.set_yticklabels([str(x) for x in yticks])
    ...
    for i in range(0,len(large_x)):
        ...
        for i in range(len(cx)):
            ax.text(cx[i]-10,80,int(cy[i]),c='blue', fontsize=14)
    ...
    for i in range(0,len(large_y2)):
        ax2.plot(large_x[i],large_y2[i],color=colors_global[i], linestyle='--')
    
    ax3 = ax.twinx()
    ax3.set_ylabel("RPS",color='red')
    ax3.set_ylim(ymin=0,ymax=20)
    yticks3 = [int(y//9) for y in yticks2]
    ax3.spines.right.set_position(("axes", 1.2))
    # ax3.set_yticks(yticks3,colors='red')

    ax3.plot(large_x[0],OVERLAYS[2],color='red')
    
    plt.savefig(f"other_wfs_scaling_plot/{wf}_alibaba.pdf", bbox_inches='tight')

def plot_step_aws_graph(OVERLAYS, wf, dynamism, colors, small_x, small_y, small_y2, aws_legend, az_legend, azv2_legend):
    fig, ax = plt.subplots()
    ax.set_ylabel("Sliding Window Avg. E2E Time (secs)",fontsize=16)
    ax.yaxis.set_minor_locator(tck.AutoMinorLocator())
    ax.xaxis.set_minor_locator(tck.AutoMinorLocator())
    ...
    label2 = '# Of Concurrent Req'
    zo = [2,1,0]
    for i in range(0,len(small_x)-2):
        l1 = ax.plot(small_x[i],small_y[i],color =colors[i],label=label1,zorder=zo[i])
    ax.set_xlim(xmin=0,xmax=840)
    ax2.set_ylim(ymin=0)
    if not yticks2 == []:
        ax2.set_yticks(yticks2)
    ...
    linestyles = ['--', '--', '-.']
    zo = [5,4,3]
    for i in range(0,len(small_y2)-2):
        l2 = ax2.plot(small_x[i],small_y2[i],color=colors_global[i], linestyle='--',label=label2,zorder=zo[i])
    ...
    ax3.plot(small_x[0],OVERLAYS[0],color='red',zorder=6)
    # ax3.plot(small_x[2],OVERLAYS[6],color='red')
    lstt = [aws_legend,az_legend,azv2_legend]
    ...
    plt.savefig(f"part32_plots_new/{wf}_medium_observation1_{dynamism}_step_aws_corrected.pdf", bbox_inches='tight')

def plot_sawtooth_aws(OVERLAYS, split, wf, dynamism, colors, medium_x, medium_y, medium_y2):
    fig, ax = plt.subplots()
    ax.set_ylabel("Sliding Window Avg. E2E Time (secs)",fontsize=16)
    ax.yaxis.set_minor_locator(tck.AutoMinorLocator())
    ax.xaxis.set_minor_locator(tck.AutoMinorLocator())
    ...
    split1,split2,split3 = split(medium_x[0],OVERLAYS[1],OVERLAYS[1])
    ax3.plot(split1[0],split1[1],color='red')
    ax3.plot(split2[0],split2[1],color='red')
    ax3.plot(split3[0],split3[1],color='red')

    ax3.hlines(xmin=split1[0][-1],xmax=split2[0][0],y=0, color='red', linestyle='-',linewidth=2)
    ax3.hlines(xmin=split2[0][-1],xmax=split3[0][0],y=0, color='red', linestyle='-',linewidth=2)

    ax3.vlines(x=split1[0][-1],ymin=0,ymax=split1[1][-1], color='red', linestyle='-',linewidth=2)
    ax3.vlines(x=split2[0][-1],ymin=0,ymax=split2[1][-1], color='red', linestyle='-',linewidth=2)
    
    ax3.vlines(x=split2[0][0],ymin=0,ymax=1, color='red', linestyle='-',linewidth=2)
    ax3.vlines(x=split3[0][0],ymin=0,ymax=1, color='red', linestyle='-',linewidth=2)

    # ax3.plot(medium_x[0],OVERLAYS[1],color='red')
    ...
    for i in range(0,len(medium_x)-2):
        print(len(medium_x[i]),len(medium_y[i]))
        # ax.plot(medium_x[i],medium_y[i],color =colors[i])
        split1,split2,split3 = split(medium_x[i],medium_y[i],OVERLAYS[i*3+1])
        ax.plot(split1[0],split1[1],color =colors[i])
        ax.plot(split2[0],split2[1],color =colors[i])
        ax.plot(split3[0],split3[1],color =colors[i])
    ax2.set_ylim(ymin=0)
    if not yticks2 == []:
        ax2.set_yticks(yticks2)
        ax2.set_yticklabels([str(x) for x in yticks2])
    ...
    ax.grid(axis="both", which="minor", linestyle="--", color="silver", linewidth = 0.2)
    for i in range(0,len(medium_y2)-2):
        split1,split2,split3 = split(medium_x[i],medium_y2[i],OVERLAYS[3*i+1])
        ax2.plot(split1[0],split1[1],color =colors[i],linestyle='--')
        ax2.plot(split2[0],split2[1],color =colors[i],linestyle='--')
        ax2.plot(split3[0],split3[1],color =colors[i],linestyle='--')
        ax2.vlines(x=split1[0][-1],ymin=0,ymax=split1[1][-1], color=colors[i], linestyle='--')
        ax2.vlines(x=split2[0][-1],ymin=0,ymax=split2[1][-1], color=colors[i], linestyle='--')
        ax2.hlines(xmin=split1[0][-1],xmax=split2[0][0],y=0, color=colors[i], linestyle='--')
        ax2.hlines(xmin=split2[0][-1],xmax=split3[0][0],y=0, color=colors[i], linestyle='--')
        ax2.vlines(x=split2[0][0],ymin=0,ymax=split2[1][0], color=colors[i], linestyle='--')
        ax2.vlines(x=split3[0][0],ymin=0,ymax=split3[1][0], color=colors[i], linestyle='--')
        # ax2.plot(medium_x[i],medium_y2[i][0:len(medium_x[i])],color=colors[i], linestyle='--')
    plt.savefig(f"part32_plots_new/{wf}_medium_observation1_{dynamism}_sawtooth_overlay_aws_corrected.pdf", bbox_inches='tight')

def plot_alibaba_aws(OVERLAYS, wf, dynamism, colors, large_x, large_y, large_y2):
    fig, ax = plt.subplots()
    ax.set_ylabel("Sliding Window Avg. E2E Time (secs)",fontsize=16)
    ax.yaxis.set_minor_locator(tck.AutoMinorLocator())
    ax.xaxis.set_minor_locator(tck.AutoMinorLocator())
    ...
    ax2 = ax.twinx()
    ax2.set_ylabel("# Of Concurrent Req")
    for i in range(0,len(large_x)-2):
        ax.plot(large_x[i],large_y[i],color =colors[i])
    ax2.set_ylim(ymin=0)
    ax.set_xlim(xmin=0,xmax=300)
    if not yticks2 == []:
    ...
    ax.grid(axis="both", which="minor", linestyle="--", color="silver", linewidth = 0.2)
    for i in range(0,len(large_y2)-2):
        ax2.plot(large_x[i],large_y2[i],color=colors[i], linestyle='--')
    plt.savefig(f"part32_plots_new/{wf}_medium_observation1_{dynamism}_alibaba_overlay_aws_corrected.pdf", bbox_inches='tight')

def plot_step_az(OVERLAYS, wf, dynamism, colors, small_x, small_y, small_y2, aws_legend, az_legend, azv2_legend):
    fig, ax = plt.subplots()
    ax.set_ylabel("E2E Time (secs)",fontsize=20)
    ax.yaxis.set_minor_locator(tck.AutoMinorLocator())
    ax.xaxis.set_minor_locator(tck.AutoMinorLocator())
    ax.grid(axis="y", which="major", linestyle="-", color="black")
    ax.grid(axis="y", which="minor", linestyle="-", color="grey")
    ...
    label1 = 'E2E Time'
    label2 = '# Of Concurrent Req'
    for i in range(1,len(small_x)-1):
    ...
    for i in range(1,len(small_y2)-1):
        l2 = ax2.plot(small_x[i],small_y2[i],color=colors_global[i], linestyle='--',label=label2)
    ...
    ax3.plot(small_x[1],OVERLAYS[3],color='red')
    # ax3.plot(small_x[2],OVERLAYS[6],color='red')
    lstt = [aws_legend,az_legend,azv2_legend]
    lsttlabel = [i.get_label() for i in lstt] + ['E2E','Num of Containers']
    lstt = lstt + [Line2D([0],[0],color="black",lw=1,linestyle='-'),Line2D([0],[0],color="black",lw=1,linestyle='--')] 
    ...
    plt.savefig(f"other_wfs_scaling_plot/{wf}_medium_observation1_{dynamism}_step_az_corrected.pdf", bbox_inches='tight')

def plot_sawtooth_az(OVERLAYS, split, wf, dynamism, colors, medium_x, medium_y, medium_y2):
    fig, ax = plt.subplots()
    ax.set_ylabel("Sliding Window Avg. E2E Time (secs)",fontsize=16)
    ax.yaxis.set_minor_locator(tck.AutoMinorLocator())
    ax.xaxis.set_minor_locator(tck.AutoMinorLocator())
    ...
    split1,split2,split3 = split(medium_x[1],OVERLAYS[4],OVERLAYS[4])
    ax3.plot(split1[0],split1[1],color='red')
    ax3.plot(split2[0],split2[1],color='red')
    ax3.plot(split3[0],split3[1],color='red')
    ...
    # ax3.plot(medium_x[1],OVERLAYS[4],color='red')
    # ax3.plot(medium_x[2],OVERLAYS[7],color='red')
    yticks2 = [int(y//5) for y in yticks]
    if not yticks == []:
        ax.set_yticks(yticks)
    ...
    for i in range(1,len(medium_x)-1):
        print(len(medium_x[i]),len(medium_y[i]))
        split1,split2,split3 = split(medium_x[i],medium_y[i],OVERLAYS[i*3+1])
        ax.plot(split1[0],split1[1],color =colors[i])
        ax.plot(split2[0],split2[1],color =colors[i])
    ...
    ax.grid(axis="both", which="minor", linestyle="--", color="silver", linewidth = 0.2)
    for i in range(1,len(medium_y2)-1):
        split1,split2,split3 = split(medium_x[i],medium_y2[i],OVERLAYS[3*i+1])
        ax2.plot(split1[0],split1[1],color =colors[i],linestyle='--')
        ax2.plot(split2[0],split2[1],color =colors[i],linestyle='--')
        ax2.plot(split3[0],split3[1],color =colors[i],linestyle='--')
        ...
        # ax2.plot(medium_x[i],medium_y2[i][0:len(medium_x[i])],color=colors[i], linestyle='--')
    plt.savefig(f"part32_plots_new/{wf}_medium_observation1_{dynamism}_sawtooth_overlay_az_corrected.pdf", bbox_inches='tight')

def plot_alibaba_az(OVERLAYS, wf, dynamism, colors, large_x, large_y, large_y2):
    fig, ax = plt.subplots()
    ax.set_ylabel("Sliding Window Avg. E2E Time (secs)",fontsize=16)
    ax.yaxis.set_minor_locator(tck.AutoMinorLocator())
    ax.xaxis.set_minor_locator(tck.AutoMinorLocator())
    ...
    ax3.plot(large_x[1],OVERLAYS[5],color='red')
    # ax3.plot(large_x[2],OVERLAYS[8],color='red')
    ax2 = ax.twinx()
    ax2.set_ylabel("# Of Concurrent Req")
    for i in range(1,len(large_x)-1):
        ax.plot(large_x[i],large_y[i],color =colors[i])
    ax2.set_ylim(ymin=0)
    ax.set_xlim(xmin=0,xmax=300)
    if not yticks2 == []:
    ...
    ax.grid(axis="both", which="minor", linestyle="--", color="silver", linewidth = 0.2)
    for i in range(1,len(large_y2)-1):
        ax2.plot(large_x[i],large_y2[i],color=colors[i], linestyle='--')
    plt.savefig(f"part32_plots_new/{wf}_medium_observation1_{dynamism}_alibaba_overlay_az_corrected.pdf", bbox_inches='tight')

def plot_step_azv2(OVERLAYS, wf, dynamism, colors, small_x, small_y, small_y2, aws_legend, az_legend, azv2_legend):
    fig, ax = plt.subplots()
    ax.set_ylabel("Sliding Window Avg. E2E Time (secs)",fontsize=16)
    ax.yaxis.set_minor_locator(tck.AutoMinorLocator())
    ax.xaxis.set_minor_locator(tck.AutoMinorLocator())
    ...
    plt.savefig(f"part32_plots_new/{wf}_medium_observation1_{dynamism}_step_azv2_corrected.pdf", bbox_inches='tight')

def plot_sawtooth_azv2(OVERLAYS, split, wf, dynamism, colors, medium_x, medium_y, medium_y2):
    fig, ax = plt.subplots()
    ax.set_ylabel("Sliding Window Avg. E2E Time (secs)",fontsize=16)
    ax.yaxis.set_minor_locator(tck.AutoMinorLocator())
    ax.xaxis.set_minor_locator(tck.AutoMinorLocator())
    ...
    # ax3.plot(medium_x[1],OVERLAYS[4],color='red')
    split1,split2,split3 = split(medium_x[2],OVERLAYS[7],OVERLAYS[7])
    ax3.plot(split1[0],split1[1],color='red')
    ax3.plot(split2[0],split2[1],color='red')
    ax3.plot(split3[0],split3[1],color='red')
    ...
    ax.grid(axis="both", which="minor", linestyle="--", color="silver", linewidth = 0.2)
    for i in range(2,len(medium_y2)):
        split1,split2,split3 = split(medium_x[i],medium_y2[i],OVERLAYS[3*i+1])
        ax2.plot(split1[0],split1[1],color =colors[i],linestyle='--')
        ax2.plot(split2[0],split2[1],color =colors[i],linestyle='--')
        ax2.plot(split3[0],split3[1],color =colors[i],linestyle='--')
        ...
        ax2.vlines(x=split3[0][0],ymin=0,ymax=split3[1][0], color=colors[i], linestyle='--')
        # ax2.plot(medium_x[i],medium_y2[i][0:len(medium_x[i])],color=colors[i], linestyle='--')
    plt.savefig(f"part32_plots_new/{wf}_medium_observation1_{dynamism}_sawtooth_overlay_azv2_corrected.pdf", bbox_inches='tight')

def plot_alibaba_azv2(OVERLAYS, wf, dynamism, colors, large_x, large_y, large_y2):
    fig, ax = plt.subplots()
    ax.set_ylabel("Sliding Window Avg. E2E Time (secs)",fontsize=16)
    ax.yaxis.set_minor_locator(tck.AutoMinorLocator())
    ax.xaxis.set_minor_locator(tck.AutoMinorLocator())
    ...
    ax.grid(axis="both", which="minor", linestyle="--", color="silver", linewidth = 0.2)
    for i in range(2,len(large_y2)):
        ax2.plot(large_x[i],large_y2[i],color=colors[i], linestyle='--')
    plt.savefig(f"part32_plots_new/{wf}_medium_observation1_{dynamism}_alibaba_overlay_azv2_corrected.pdf", bbox_inches='tight')
...
if __name__ == "__main__":
    ...
    plot_step_graph_all_csps(wf, dynamism, colors, small_x, small_y, small_y2, aws_legend, az_legend, azv2_legend)
    plot_sawtooth_graph_all_csps(OVERLAYS, split, wf, dynamism, colors, medium_x, medium_y, medium_y2)
    plot_alibaba_graph_all_csps(wf, dynamism, colors, large_x, large_y, large_y2)
    
    # plot_step_aws_graph(OVERLAYS, wf, dynamism, colors, small_x, small_y, small_y2, aws_legend, az_legend, azv2_legend)
    # plot_sawtooth_aws(OVERLAYS, split, wf, dynamism, colors, medium_x, medium_y, medium_y2)
    # plot_alibaba_aws(OVERLAYS, wf, dynamism, colors, large_x, large_y, large_y2)
    
    plot_step_az(OVERLAYS, wf, dynamism, colors, small_x, small_y, small_y2, aws_legend, az_legend, azv2_legend)
    # plot_sawtooth_az(OVERLAYS, split, wf, dynamism, colors, medium_x, medium_y, medium_y2)
    # plot_alibaba_az(OVERLAYS, wf, dynamism, colors, large_x, large_y, large_y2)

    
    plot_step_azv2(OVERLAYS, wf, dynamism, colors, small_x, small_y, small_y2, aws_legend, az_legend, azv2_legend)
    # plot_sawtooth_azv2(OVERLAYS, split, wf, dynamism, colors, medium_x, medium_y, medium_y2)
    # plot_alibaba_azv2(OVERLAYS, wf, dynamism, colors, large_x, large_y, large_y2)
...
# 3eeb8f2f-d31a-4fe0-a439-9bf4c9126cad,azure_v2,centralindia,8,300,medium,alibab
...
74c113ab-3385-41a1-8cc5-613f393af065,azure,centralindia,8,300,medium,sawtooth
2fa4cb97-2bac-4f22-b568-15cf9d88a052,azure,centralindia,8,300,medium,alibaba
3d2c7e7d-5520-4b86-ab88-98f4c003a82e,azure_v2,centralindia,8,300,medium,step_function
712117f3-11c1-4973-a683-55ad8ce0dc1c,azure_v2,centralindia,8,300,medium,sawtooth
a6252222-e64f-4eca-bc6f-ca8253f69bc4,azure_v2,centralindia,8,300,medium,alibaba
...
7c40fda9-5263-4b13-8613-d472241f1fa3,aws,centralindia,8,300,medium,sawtooth
c29b8954-1e4c-4d71-9713-cc452628df1f,aws,centralindia,8,300,medium,alibaba
29aae4bc-7ad4-4c88-8ab0-3834abdb98af,azure,centralindia,8,300,medium,step_function
74c113ab-3385-41a1-8cc5-613f393af065,azure,centralindia,8,300,medium,sawtooth
2fa4cb97-2bac-4f22-b568-15cf9d88a052,azure,centralindia,8,300,medium,alibaba
d0ff194c-1242-412c-ac22-0672eff097a0,azure_v2,centralindia,8,300,medium,step_function
3c4d6c54-e970-4fba-8aad-d132c9127509,azure_v2,centralindia,8,300,medium,sawtooth
...
2a:
6b0a3097-361d-488e-b4ff-c38bb7f35ab6,azure,centralindia,8,300,medium,step_function

2b:

math

e23d91e9-0c7d-4866-8a16-ed312fce3613
