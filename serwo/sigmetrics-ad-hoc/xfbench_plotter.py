...
class XFBenchPlotter:
    ...
    def __init__(self, exp_directory: str, format: str):
        self.__xfaas_dag = self.DagLoader(pathlib.Path(exp_directory) / "dag.json").get_dag()
        self.__format = format

        '''
        Constructed directories and paths
        '''
        self.__exp_directory = pathlib.Path(exp_directory)
    ...
    def plot_comms_time_payload_varition(self, yticks: list):
        experiment_config = self.__exp_conf
        e2etime_payload_variation = defaultdict(list)
        sid_payload_map = dict()
        
        for sid in experiment_config:
            sid_payload_map[sid] = experiment_config[sid]["payload_size"]

        logs = self.__get_provenance_logs()
        sink_node = [node for node in self.__xfaas_dag.nodes if self.__xfaas_dag.out_degree(node) == 0][0]
        for log in logs:
            e2etime_payload_variation[sid_payload_map[log["session_id"]]].append((log["functions"][sink_node]["end_delta"])/1000)

        for key, val in e2etime_payload_variation.items():
            logger.info(f"Median E2E {key} - {statistics.median(val)}")
        ...
        ##### VLINES #####        
        # add lighter vlines between the boxes themselves
        _xloc = ax.get_xticks()[0: len(labels)]
        vlines_x_between = []
        for idx in range(0, len(_xloc)-1):
            vlines_x_between.append(_xloc[idx]/2 + _xloc[idx+1]/2)
        ax.vlines(x=vlines_x_between, ymin=0, ymax=ax.get_ylim()[1], linestyles='solid', color='darkgrey', linewidth=1.5)
        ...
        fig.savefig(self.__plots_dir / f"payload_variation_e2e_box_{self.__get_outfile_prefix()}.{self.__format}", bbox_inches='tight')
