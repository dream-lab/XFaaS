...
RUNNER_TEMPLATE_DIR = pathlib.Path.joinpath(PARENT_DIRECTORY_PATH,"python/src/runner-templates/aws")
YAML_TEMPLATE_DIR = pathlib.Path.joinpath(PARENT_DIRECTORY_PATH, "python/src/faas-templates/aws/yaml-templates")
...
CREATE_ENVIRONMENT_SCRIPT = pathlib.Path.joinpath(PARENT_DIRECTORY_PATH, "scripts/bash/aws/create_env.sh").absolute().resolve()
CREATE_LAMBDA_FUNCTION_SCRIPT = pathlib.Path.joinpath(PARENT_DIRECTORY_PATH, "scripts/bash/aws/create_lambda_fn.sh").absolute().resolve()
DEPLOY_STEPFUNCTION_SCRIPT = pathlib.Path.joinpath(PARENT_DIRECTORY_PATH, "scripts/bash/aws/deploy_sfn.sh").absolute().resolve()
...
def template_function_id(runner_template_dir, function_id, function_name):
    ...
    try:
        ...
        print(f"Created jinja2 environement for templating AWS function ids for function::{function_name}")
    except:
        raise Exception(f"Unable to load environment for AWS function id templating for function::{function_name}")
    ...
    try:
        ...
        with open(f"{runner_template_dir}/{runner_template_filename}", "w") as out:
            out.write(output)
            print(f"Rendered and flushed the runner template for AWS function for function::{function_name}")
    except Exception as e:
        print(e)
        raise Exception(f"Unable to render the function runner template for AWS function for function::{function_name}")
...
for function_metadata in function_metadata_list:
    ...
    function_runner_filename = function_object_map[function_metadata['name']].get_runner_filename()
    function_path = function_object_map[function_metadata['name']].get_path()
    function_module_name = function_object_map[function_metadata['name']].get_module_name()
    ...
    runner_template_filename = template_function_id(runner_template_dir=RUNNER_TEMPLATE_DIR, function_id=function_id, function_name=function_name)
    os.system(f"{CREATE_LAMBDA_FUNCTION_SCRIPT} {os.path.join(PARENT_DIRECTORY_PATH,function_path)} {AWS_BUILD_DIR} {function_name} {function_module_name} {function_runner_filename} {RUNNER_TEMPLATE_DIR} {SERWO_UTILS_DIR} {runner_template_filename}")
    os.system(f"rm -r {RUNNER_TEMPLATE_DIR}/{runner_template_filename}")

# generate asl template
try:
    AWSSfnYamlGenerator.generate_sfn_yaml(function_metadata_list, statemachine, function_object_map,YAML_TEMPLATE_DIR, AWS_BUILD_DIR, YAML_FILE, TRIGGER_TYPE)
except Exception as e:
    print(e)

# TODO [TK] - add validator for generated yaml
print("Building Statemachines JSON..")
AWSSfnAslBuilder.generate_statemachine_json(statemachine_structure, AWS_BUILD_DIR, JSON_FILE)

print("Adding API specification to user directory")
os.system(f"cp {YAML_TEMPLATE_DIR}/execute-api-openapi.yaml {AWS_BUILD_DIR}/{statemachine['api_file']}")
...
os.system(f"{DEPLOY_STEPFUNCTION_SCRIPT} {AWS_BUILD_DIR} {SAM_BUILD_DIR}  {SAM_STACKNAME} {REGION} {YAML_FILE}")

print(f"Writing SAM outputs to .. {OUTPUTS_FILEPATH}")
os.system(f'aws cloudformation describe-stacks --stack-name {SAM_STACKNAME} --query \"Stacks[0].Outputs\" --output json > {OUTPUTS_FILEPATH}')
