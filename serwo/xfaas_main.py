...
parser.add_argument("--wf-user-directory", dest='wf_user_directory',
                    type=str, help="Workflow user directory")
parser.add_argument("--dag-benchmark", dest='dag_benchmark',
                    type=str, help="Path DAG Benchmark")
parser.add_argument("--dag-file-name", dest='dag_filename',
                    type=str, help="DAG FILE NAME")
parser.add_argument("--is-async", dest='is_async',
...
def add_collect_logs(dag_definition_path, user_wf_dir, xfaas_user_dag, region, part_id, csp):
    # Skip Azure logging setup if not deploying to Azure
    if csp.lower() != 'azure':
        print(f"Skipping Azure logging setup - deploying to {csp}")
        return None
    ...
    new_collect_logs_dir = os.path.abspath(f'{user_wf_dir}/CollectLogs')
    ...
    try:
        ...
        print("AZURE_SUBSCRIPTION_ID environment variable not set. Skipping Azure logging setup.")
    ...
    try:
        ...
        stream = os.popen(
            f'az storage account show-connection-string --name {storage_account_name} --resource-group {resource_group_name}')
    ...
    fin_dict = {'queue_name': queue_name, 'connection_string':
                jsson['connectionString'], 'storage_account': storage_account_name, 'group': resource_group_name}
...
def generate_new_dags(partition_config, xfaas_user_dag, user_wf_dir, dag_definition_path):
    ...
    for edge in subdag.edges:
        dagg['Edges'].append({nx_dag.nodes[edge[0]]['NodeName']: [
                             nx_dag.nodes[edge[1]]['NodeName']]})
    ...
    for i in range(1, len(partition_config)):
        ...
        if out_degree > 1:
            ...
            forward_fn_dir = os.path.abspath(f"{user_wf_dir}/ForwardFunction")
            ...
            for edge in out_edges:
                dagg['Edges'].append(
                    {forward_fn_name: [nx_dag.nodes[edge[1]]['NodeName']]})
...
def run(user_wf_dir, dag_definition_file, benchmark_file, csp, region):
    ...
    # 1 : Partition_config created for all nodes. Decides where to place each node - Which csp based on the
    # optimization algorithm that uses the benchmarks calculated from XBench.
    if len(disabled_pair) > 0:  # This was just initialized to 0..
        partition_config = xfaas_optimizer.optimize(
            xfaas_user_dag, user_pinned_nodes, benchmark_path, disabled_pair=disabled_pair)
    else:
        partition_config = xfaas_optimizer.optimize(
            xfaas_user_dag, user_pinned_nodes, benchmark_path)

    # breakpoint()

    # Creates DAGs based on the partition config created in the previous step.
    # Saves the new DAGs in user_wf_dir/partitions/csp-region-partid/dag.json
    generate_new_dags(partition_config, xfaas_user_dag,
                      user_wf_dir, dag_definition_path)
    ...
    for p in partition_config:
        ...
        print(p.get_function_name(), p.get_part_id(),
              p.get_left_csp().get_name(), p.get_region())
    ...
    # The last partition is identified and collect logs node is added to it.
    # Each function calculates its logs and passes it to the next function.
    # Finally all the logs are collected, aggregated and pushed to the monitoring interface.
    # This is only done for Azure... TODO: Check why AWS is skipped.
    queue_details = add_collect_logs(
        updated_dag_definition_path, updated_wf_dir, xfaas_user_dag, region, part_id, csp)
    ...
    refactored_wf_id = xfaas_provenance.push_refactored_workflow(
        "dag.json", user_wf_dir, wf_id, csp)
    wf_deployment_id = xfaas_provenance.push_deployment_logs(
        "dag.json", user_wf_dir, wf_id, refactored_wf_id, csp)
    xfaas_resource_generator.generate(
        user_wf_dir, partition_config, "dag.json")
