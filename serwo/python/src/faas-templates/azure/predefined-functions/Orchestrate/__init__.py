...
def orchestrator_function(context: df.DurableOrchestrationContext):
    ...
    # user dag execution
    pyee = yield context.call_activity("TaskA", serwoObject)
    vbqu = yield context.call_activity("TaskB", pyee)
    rbzq = yield context.call_activity("TaskC", vbqu)
    bqzn = yield context.call_activity("TaskD", rbzq)
    cyvt = yield context.call_activity("TaskM", bqzn)
    tkab = yield context.call_activity("TaskN", cyvt)
    leaf = yield context.call_activity("TaskE", rbzq)
    whtv = yield context.call_activity("TaskF", leaf)
    jumf = yield context.call_activity("TaskI", whtv)
    hlni = []
    clsf = context.call_activity("TaskG", whtv)
    fyjp = context.call_activity("TaskH", whtv)
    vhpf = context.call_activity("TaskJ", jumf)
    hlni.append(clsf)
    hlni.append(fyjp)
    hlni.append(vhpf)
    jpnt = yield context.task_all(hlni)
    bbna = []
    lxin = context.call_activity("TaskO", tkab)
    qfcw = context.call_activity("TaskK", jpnt)
    bbna.append(lxin)
    bbna.append(qfcw)
    huqp = yield context.task_all(bbna)
    strj = yield context.call_activity("TaskL", huqp)
    gpzu = yield context.call_activity("TaskP", strj)
    psgc = yield context.call_activity("TaskQ", gpzu)
    psgc = insert_end_stats_in_metadata(psgc)
    uxhq = yield context.call_activity("CollectLogs", psgc)
    return uxhq
