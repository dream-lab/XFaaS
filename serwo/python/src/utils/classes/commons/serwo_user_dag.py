...
class SerWOUserDag:
    ...
    def __create_egress_node(
        self, partition_point_id, new_node_params, forward_function_params
    ):
        outG = self.__dag
    ...
    def get_partition_points_after_user_pinning(
        self, partition_points, user_pinned_nodes, num_parts, user_pinned_csp
    ):
    ...
    def handle_two_partitions(self, partition_points, user_pinned_nodes):
        ...
        for partition_point in partition_points:
            left_partition, right_partition = get_partition_lists(
                partition_point, u_graph
            )
            part_id = is_partition_valid(
                left_partition, right_partition, user_pinned_nodes
            )
            if part_id != -1:
                np = partition_point
            ...
            if u not in is_exists:
                ...
                fin_list.append(
                    dict(
                        ...
                        in_degree=in_deg,
                        left_csp=CSP.toCSP("AWS"),
                        right_csp=CSP.toCSP("Azure"),
                    )
                )
    ...
    def get_best_partition(
        self, partition_points, num_parts, dag_path, user_pinned_csp, user_pinned_nodes
    ):
        return serwo_benchmark_evaluator.get_best_partition_point(
            u_graph=self.__dag,
    ...
    def get_partitioned_graph(
        self,
        partition_point: PartitionPoint,
        new_node_params: dict,
        forward_function_params: dict,
    ):
        ...
        source_nodeId = [
            node for node in self.__dag.nodes if self.__dag.in_degree(node) == 0
        ][0]
        sink_nodeId = [
            node for node in self.__dag.nodes if self.__dag.out_degree(node) == 0
        ][0]
        ...
        paths_between_source_egress = nx.all_simple_paths(
            self.__dag, source=source_nodeId, target=egressNodeId
        )

        paths_between_egress_sink = nx.all_simple_paths(
            self.__dag, source=egressNodeId, target=sink_nodeId
        )

        # partition the graph based on nodes
        outG = self.get_dag()
        nodes_between_source_and_egress = {
            node for path in paths_between_source_egress for node in path
        }
        nodes_between_egress_and_sink = {
            node for path in paths_between_egress_sink for node in path
        }
        ...
        for u, v in list(left_subgraph.edges()):
            print(
                left_subgraph.nodes[u]["NodeName"], left_subgraph.nodes[v]["NodeName"]
            )

        print("Right subgraph edges")
        for u, v in list(right_subgraph.edges()):
            print(
                right_subgraph.nodes[u]["NodeName"], right_subgraph.nodes[v]["NodeName"]
            )
