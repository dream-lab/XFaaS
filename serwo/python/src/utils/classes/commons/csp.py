...
class CSP:
    ...
    def build_resources(self, user_dir, dag_definition_path, region, part_id, dag_definition_file, is_netherite):
        if self.__name == 'azure' or self.__name == 'azure_v2':
            if self.__name == 'azure_v2':
                is_netherite = True
            self.build_az(dag_definition_file, dag_definition_path,
                          part_id, region, user_dir, is_netherite)
        if self.__name == 'aws':
            if part_id == "0000":  # first partition point is REST
                aws_deployer = AWS(
                    user_dir, dag_definition_file, "REST", part_id, region)
                aws_deployer.build_resources()
                aws_deployer.build_workflow()
                aws_deployer.deploy_workflow()
            else:
                aws_deployer = AWS(
                    user_dir, dag_definition_file, "SQS", part_id, region)
                aws_deployer.build_resources()
                aws_deployer.build_workflow()
                aws_deployer.deploy_workflow()
    ...
    def build_az(self, dag_definition_file, dag_definition_path, part_id, region, user_dir, is_netherite):
        ...
        azure_resource_generator.generate(
            user_dir, dag_definition_path, region, part_id, is_netherite)
        print(':'*80, 'Azure Build')
        azure_builder.build(user_dir, dag_definition_file,
                            region, part_id, is_netherite)
