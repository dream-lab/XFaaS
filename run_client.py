...
# print(args)
file_name = (
    f"serwo/examples/{args.exp_name}/build/workflow/resources/deployment-structure.json"
)
print(file_name)
f = open(file_name, "r")
...
if entry_csp == "aws":
    ...
    for json_object in resources:
        if "AWSArn" in json_object["OutputKey"]:
            template = template.replace(
                "{{statemachinearn}}", json_object["OutputValue"]
            )
        if "ExecuteApi" in json_object["OutputKey"]:
            template = template.replace(
                "{{execurl}}", json_object["OutputValue"] + "/execute"
            )
    ...
    resources_file_name = (
        f"serwo/examples/{args.exp_name}/build/workflow/resources/azure_resources.json"
    )
    f = open(resources_file_name, "r")
    resources = json.load(f)
    f.close()
    template = template.replace(
        "{{execurl}}",
        f'https://{resources["app_name"]}.azurewebsites.net/api/orchestrators/Orchestrate',
    )
else:
    print("Incorrect CSP")
    exit(1)
...
subprocess.run(
    [
        f"jmeter -n -t serwo/examples/{args.exp_name}/build/workflow/resources/jmx_client.jmx -l smart-grid-singlecloud-azure.jtl"
    ],
    shell=True,
)
